...
void setInputState()
{
	...
	InputState[Now][KEY_LEFT] = GetAsyncKeyState(VK_LEFT) < 0;
	InputState[Now][KEY_RIGHT] = GetAsyncKeyState(VK_RIGHT) < 0;
	...
	InputState[Now][MOUSE_X] = static_cast<short>(pt.x);
	InputState[Now][MOUSE_Y] = static_cast<short>(pt.y);
}
//指定したキーやボタンが押されているか
...
void createInput()
{
	HWnd = FindWindow(L"GAME_WINDOW",NULL);
	hideCursor();
	//ウィンドウ中央にマウスをセット
	//RECT rect;
